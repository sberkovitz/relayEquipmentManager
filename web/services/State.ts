...
export class StateRoute {
    public static initRoutes(app: express.Application) {
        ...
        app.get('/devices/all', (req, res) => {
            ...
            } catch (err) { logger.error(`/devices/all Error getting pins: ${err.message}`); }
            try {
            ...
            for (let i = 0; i < cont.i2c.buses.length; i++) {
                ...
                for (let j = 0; j < bus.devices.length; j++) {
                    ...
                    } catch (err) { logger.error(`/devices/all Error getting i2c-${bus.id}: ${err.message}`); }
                }
            }
            for (let i = 0; i < cont.oneWire.buses.length; i++) {
                ...
                } catch (err) { logger.error(`/devices/all Error getting 1-wired ${bus.id}: ${err.message}`); }
            }
            try {
            ...
            } catch (err) { logger.error(`/devices/all Error getting generic: ${err.message}`); }
